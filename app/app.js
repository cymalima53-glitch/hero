...
async function loadFiles() {
    try {
        const res = await fetch(`/api/files/${state.currentLang}`);
...
async function loadContent() {
    try {
        const res = await fetch(`/data/${state.currentLang}`);
        const json = await res.json();
...
function renderStudents() {
    ...
    state.students.forEach(s => {
        ...
        tr.innerHTML = `
            ...
            <td style="padding:1rem; color:var(--text-muted); font-size:0.9rem;">${s.id}</td>
